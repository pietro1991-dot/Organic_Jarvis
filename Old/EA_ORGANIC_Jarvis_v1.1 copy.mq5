...
//|   RANDOM (center±margin):           Mediana (robusto)           |
...
input int    BuyTrailingStartPips = 20;         // 📈 BUY Trailing: attiva dopo X pips profitto (0=disattivato)
input int    BuyTrailingStepPips  = 10;         // 📈 BUY Trailing: mantieni SL a Y pips dal prezzo corrente
...
input int    SellTrailingStartPips = 20;        // 📉 SELL Trailing: attiva dopo X pips profitto (0=disattivato)
input int    SellTrailingStepPips  = 10;        // 📉 SELL Trailing: mantieni SL a Y pips dal prezzo corrente
...
// Il lookback emerge dal periodo naturale stesso: lookback = periodo × scale²(H) × φ
...
// scale(H) = base^H dove base è empirica (o 2 di default)
...
// decay(H) = base^(-H) = 1/scale(H) dove base è empirica
...
//| Elimina 30% più vecchio per evitare crash memoria                |
//+------------------------------------------------------------------+
void CleanupHistoryBuffer(double &buffer[], int &size, int &index, double &sum, double &sumSq)
{
    ...
    // Questa funzione ora è un no-op per sicurezza
    return;
}
...
void UpdateGlobalHurst()
{
    ...
    if (count > 0) {
        ...
        // 📊 CHECK RESIZE DINAMICO: verifica se Hurst è cambiato significativamente
        // Soglia = 1σ(H) invece di 2σ per triggare resize più frequente (metà confidenza)
...
// ✅ DATA-DRIVEN: Soglia ridimensionamento = 2σ(H) storico (cambio regime a 95% confidenza)
// Non più valore fisso 0.15, ma derivato dalla variabilità empirica di Hurst
...
int OnInit()
{
    ...
    HURST_HISTORY_MAX = 64;  // Fisso a 64 per performance e stabilità
    ...
    // Base sistema = minimo tra TF attivi (più reattivo)
...
void RecalculateOrganicSystem()
{
    ...
    if (needFullHurstRecalc) {
        ...
        if (g_dataReady_H1 && tfData_H1.atr_avg > 0) {
            ...
            // Fallback = periodo H1 empirico × scale²(H) ≈ 48 pips
    ...
    if (EnableHurstFilter) {
        ...
        // g_hurstComposite è già validato dal range [HURST_RANGE_MIN, HURST_RANGE_MAX] in CalculateHurstExponent
        // DEBUG LIMITATO: stampa qualche volta anche in backtest per capire perché il buffer resta a 1/33
...
double CalculateHurstExponent(MqlRates &rates[], int n)
{
    ...
    // 🔧 MODIFICA: Non limitiamo più il massimo - usa tutte le barre disponibili
    // Più dati = calcolo H più stabile e affidabile
    ...
    // 🌱 WARMUP ADATTIVO: se abbiamo meno barre del minimo ideale, usa tutto ciò che è disponibile
    // purché sia >= 64 barre (minimo per R/S con almeno 2 scale valide)
    if (n < minBarsHurst) {
...
void PreloadHurstBufferFromHistory()
{
    ...
    // ⏱️ TIMER: Misura tempo totale preload
    ...
    for (int sample = 0; sample < effectiveSamples; sample++) {
        ...
        if (copiedD1 >= minBarsD1) {
            ...
            if (idxD1 < copiedD1 - bufD1) {
                ...
                int barsD1 = MathMin(64, copiedD1 - idxD1);  // 🔧 MAX 64 per velocità
                // 🔧 RIUSA array pre-allocato
        ...
        if (weightSum > 0) {
            ...
            // Il range Hurst è DERIVATO dai dati, non imposto a priori!
            // I valori Hurst sono già validati in CalculateHurstExponent
    ...
    if (g_hurstHistorySize == 0) {
        ...
        g_hurstStdev = (GetHurstMax() - GetHurstMin()) / 4.0;  // Range/4 ≈ 1σ per distribuzione uniforme
...
double CalcYoudenThreshold()
{
    ...
    // Minimo step = decay³(H) empirico per granularità sufficiente
...
//| Chiamata quando Hurst cambia significativamente (ΔH > threshold) |
...
void ResizeDynamicBuffers()
{
    ...
    if (newMomentumSize != g_lastMomentumBufferSize) {
        ...
        // Ripristina dati (prendi gli ultimi N se nuovo buffer più piccolo)
...
{
    // Verifica se Hurst è cambiato significativamente
    double deltaH = MathAbs(g_hurstGlobal - g_lastHurstForResize);
    
    // ✅ DATA-DRIVEN: Soglia = 2σ(H) storico (95% confidenza = cambio regime)
    double minThreshold = GetOrganicDecayPow(GetDefaultHurst(), 2.0);  // decay²(H) empirico
...
int UpdateScoreMomentum(double currentScore)
{
    ...
    // IMPORTANTE: Usa ArraySize() per dimensione reale, non GetBufferXLarge() che può cambiare!
...
{
    // IMPORTANTE: Usa ArraySize() per dimensione reale, non GetBufferMedium() che può cambiare!
...
int GetReversalSignal(double &strength)
{
    ...
    if (reversalBufferMax <= 0) return 0;  // Safety check
    ...
    // Richiede minimo = periodo × decay²(H) (data-driven)
    // ---------------------------------------------------------------
    int basePeriod = (g_naturalPeriod_Min > 0) ? g_naturalPeriod_Min : BOOTSTRAP_MIN_BARS;
    int minSamples = MathMax(4, (int)MathRound(basePeriod * GetOrganicDecayPow(GetDefaultHurst(), 2.0)));
    ...
    if (g_reversalHistorySize >= minSamples) {
        ...
        if (!g_reversalThresholdReady) {
            ...
            // Log update periodico (ogni periodo naturale)
...
NaturalPeriodResult CalculateNaturalPeriodForTF(ENUM_TIMEFRAMES tf)
{
    ...
    // 2. Dopo primo calcolo: lookback = periodo_naturale × scale²(H) × φ
    ...
    // purché siano almeno pari al minimo statistico
    ...
    // 🌱 maxLag = barre × decay²(H) (data-driven: più memoria in regime trending)
    ...
    //   H < g_hurstRandomLow: mean-reverting → peso maggiore
    // 
    // 🔧 USA TUTTE LE BARRE COPIATE (rates[] già disponibile)
    double hurstValue;
    ...
    // 🌱 MINIMO PERIODO: almeno BOOTSTRAP_MIN_BARS per stabilità
    ...
    // 🌱 LIMITI EMPIRICI: minimo BOOTSTRAP_MIN_BARS×2, massimo per evitare overflow
...
//|                                                                  |
//| TRENDING (H > center+margin):     EMA (recency bias)             |
//| RANDOM (center±margin):           Mediana (robustezza)           |
...
bool CalculateEmpiricalThresholds(TimeFrameData &data, int lookback)
{
    ...
    if (ArraySize(data.adx) >= n) {
        ...
        if (adx_range < 0.1) {  // ADX piatto o quasi (< 0.1 punti di variazione)
            ...
            // H trending (>0.5) → range più stretto (es. 30-70)
            // H random (=0.5) → range standard (25-75)
            // H mean-rev (<0.5) → range più largo (es. 20-80)
...
void CalculateOrganicPeriodsFromData(ENUM_TIMEFRAMES tf, OrganicPeriods &organic, int naturalPeriod, double weight, double hurstExp)
{
    ...
    // - Trend indicators: minimo BOOTSTRAP_MIN_BARS×2 (16) per filtrare rumore
...
//| Ritorna true se almeno un periodo è cambiato > decay(H)          |
...
bool PeriodsChangedSignificantly()
{
    ...
    //    Per H=0.5 → decay≈0.707 = 70.7% (molto conservativo)
    //    Per H=0.7 → decay≈0.616 = 61.6% (trending, meno sensibile)
    //    Per H=0.3 → decay≈0.812 = 81.2% (mean-reverting, più sensibile)
...
bool LoadTimeFrameData(ENUM_TIMEFRAMES tf, TimeFrameData &data, int bars)
{
    ...
    // Se abbiamo meno barre del minimo, tenta un secondo caricamento (start=0) e poi rinuncia senza spam
    if (copiedBars < barsToCheck) {
        int copiedBarsSecond = CopyRates(_Symbol, tf, 0, bars, data.rates);
        if (copiedBarsSecond > copiedBars) copiedBars = copiedBarsSecond;
        if (copiedBars < barsToCheck) {
            int idx = MathMin(31, (int)tf % 32);
            if (warnInsufficientData[idx] < 3) {
                PrintFormat("[WARN] TF %s: Dati insufficienti (%d/%d barre). Ritento al prossimo tick.",
...
void CalculateOrganicValues(TimeFrameData &data, int count, int minBarsRequired)
{
    ...
    if (count < minBarsRequired) {
        // 🔧 Log più diagnostico per capire il blocco
...
void OnTick()
{
    ...
    // 🛡️ Exit anticipato su segnali reversal contrari (taglia perdite grosse)
    ...
    // Se servono più barre di quelle disponibili, usa il massimo disponibile
...
//| Logica: Dopo X pips profitto, mantieni SL a Y pips dal prezzo   |
//+------------------------------------------------------------------+
void UpdateTrailingStops()
{
    // Check se almeno uno dei trailing è attivo
    if (BuyTrailingStartPips <= 0 && SellTrailingStartPips <= 0) return;
    ...
    for (int i = 0; i < totalPositions; i++) {
        ...
        // Determina parametri trailing per questo tipo
        int trailingStart = (type == POSITION_TYPE_BUY) ? BuyTrailingStartPips : SellTrailingStartPips;
        int trailingStep = (type == POSITION_TYPE_BUY) ? BuyTrailingStepPips : SellTrailingStepPips;
        ...
        // Calcola profitto in pips
        double profitPips = 0;
        if (type == POSITION_TYPE_BUY) {
            profitPips = (currentPrice - openPrice) / point / 10; // Converti a pips standard
        } else {
        ...
        double newSL = 0;
        if (type == POSITION_TYPE_BUY) {
            // BUY: SL = Bid - trailingStep pips
            newSL = currentPrice - (trailingStep * 10 * point);
            newSL = NormalizeDouble(newSL, digits);
            
            // Aggiorna solo se nuovo SL è migliore (più alto)
            if (currentSL > 0 && newSL <= currentSL) continue;
            if (newSL <= openPrice) continue; // Non mettere SL sotto breakeven
        } else {
            // SELL: SL = Ask + trailingStep pips
            newSL = currentPrice + (trailingStep * 10 * point);
            newSL = NormalizeDouble(newSL, digits);
            
            // Aggiorna solo se nuovo SL è migliore (più basso)
            if (currentSL > 0 && newSL >= currentSL) continue;
        ...
        if (trade.PositionModify(ticket, newSL, currentTP)) {
            PrintFormat("[TRAILING] Aggiornato SL posizione #%I64u %s | Profit: %.1f pips | Nuovo SL: %.5f (step %d pips)",
                ticket, type == POSITION_TYPE_BUY ? "BUY" : "SELL", 
                profitPips, newSL, trailingStep);
        } else {
            int errCode = GetLastError();
            // Log errore solo se non è "no changes" (codice 10025)
            if (errCode != 10025) {
...
//+------------------------------------------------------------------+
//| 🛡️ EXIT ANTICIPATO: Chiude posizioni in perdita su segnale contrario forte |
...
void CheckEarlyExitOnReversal()
{
    ...
    for (int i = totalPositions - 1; i >= 0; i--) {
        ...
        // Verifica se segnale reversal è CONTRARIO alla posizione
                        ...
                        const MqlTradeResult& result)
{
    ...
    if (tradesMax > 0) {
        ...
        // NOTA: scoreAtEntry già recuperato e loggato nel blocco sopra
