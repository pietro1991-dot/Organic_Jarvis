...
//|                                                                 |
//| SOGLIE DINAMICHE:                                               |
//|   ADX threshold = avg + (1/f) � stddev (dai dati)               |
//|   Score threshold = mean + stdev � f?� (dai dati)               |
//|   Zona Hurst = centro � stdev � f?� (dai dati)                  |
//|                                                                 |
//| READY CHECK:                                                    |
...
int OnInit()
{
    ...
    // Warmup = f� � naturalPeriod pi� lungo disponibile (minimo 50 barre)
...
void RecalculateOrganicSystem()
{
    ...
    if (needFullHurstRecalc) {
        ...
        // Se s�, ricrea gli handle indicatori con i nuovi periodi
        if (PeriodsChangedSignificantly()) {
            ...
            if (!InitializeIndicators()) {
                ...
                // ?? FIX: Invalida cache dopo ricreazione handle - i dati vecchi non sono pi� validi
...
//| ?? FLOOR ORGANICO: Mai sotto f?� � 62% (sezione aurea)           |
//+------------------------------------------------------------------+
void UpdateDynamicThreshold()
{
    ...
    // ?? FIX: Minimo organico ridotto = f4 � 7 (invece di f5 � 11) per trading pi� veloce
    int minSamplesOrganic = (int)MathRound(MathPow(PHI, 4));  // f4 � 7
    // ?? FIX: Usa f?� invece di f?� � f?� per minimo pi� raggiungibile
    ...
    // 
    // FASE 1 (pochi dati): Floor pragmatico = f?� � 62%
    // FASE 2 (abbastanza dati): Floor data-driven = percentile 62� degli score
    // 
    // Formula finale: max(floor, mean + stdev � f)
    // 
    // Minimo per percentile = f6 � 18 campioni (significativit� statistica)
    // ---------------------------------------------------------------
    
    double floorThreshold = 0.0;
    string floorType = "";
    
    int minForPercentile = (int)MathRound(MathPow(PHI, 6));  // f6 � 18
    
    if (g_scoreHistorySize >= minForPercentile) {
        // -----------------------------------------------------------
        // ?? FASE 2: FLOOR DATA-DRIVEN
        // Percentile f?� � 100 � 62� percentile degli score storici
        ...
        // Usa f?� � 100 � 62% come floor fisso temporaneo
        // Verr� sostituito dal percentile quando avremo abbastanza dati
        // -----------------------------------------------------------
        floorThreshold = PHI_INV * 100.0;  // f?� � 100 � 61.8%
        floorType = StringFormat("FALLBACK: f?� fisso (attesa %d/%d campioni per data-driven)", g_scoreHistorySize, minForPercentile);
    }
    
    double dataThreshold = mean + stdev * PHI;  // mean + stdev � f
    ...
    // ? VALIDATO: Ceiling organico a 1-f?� � 76.4% - non troppo restrittivo
    ...
    // ?? Log solo se cambio significativo (stdev � f?� = cambio minimo per log)
...
//|    - g_dynamicThreshold � gi� validato nel range [23.6%, 76.4%]  |
//|    - ScoreThreshold � input utente (potenzialmente qualsiasi)    |
//+------------------------------------------------------------------+
double GetCurrentThreshold()
{
    if (AutoScoreThreshold) {
        // ? Se la soglia automatica non � ancora pronta, usa quella manuale come fallback
        // Cos� il trading pu� iniziare subito invece di aspettare warm-up
    ...
    return ScoreThreshold;  // ?? Input utente, non validato
}
...
bool CalculateEmpiricalThresholds(TimeFrameData &data, int lookback)
{
    ...
    if (obvSize >= n && n > 1) {
        ...
        if (obv_change_stdev > 0) {
            ...
            // Scala = range / (f� � sqrt(n)) come stima della variazione tipica
...
// ?? NOTA: Usa direttamente enableXXX (bool globali) e data.organic.weight (peso organico TF)
{
    ...
    if (enableIchimoku && ArraySize(data.ichimoku_tenkan) > lastIdx && 
        ...
        if (price > cloud_top) {
            ...
            cloud_signal = MathMax(-0.5, MathMin(0.5, cloud_signal));  // Dentro cloud = max �0.5
        }
